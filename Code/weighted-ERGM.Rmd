...
# se une todo para asegurarnos que los atributos de los nodos y el orden de los nodos se mantiene
...
# en igraph los nodos de la red se anotan con V
# añado el nombre de los atributos y pongo NA para los polinizadores para que no le de el nombre de los 
# atributos. Se podría usar dim1 o dim2 para sustituir 1:60 o 21. 
...
# (v/n) --> atributo de vertice, variable cuantitativa
# (v/l) --> atributo de vertice, label
# (v/c) --> atributo de vertice, variable categorica
# (e/n) --> atributo de edge, variable cuantitativa
...
After creating the igraph object with associated node information, I have to create an adjacency matrix from the igraph object prior to obtaining the analogous network object. From the igraph object, I also extracted all plant attribute variables to set vertex (i.e. nodes) attributes in the network object. ERGMs work with network objects.
...
ergm0 <- formula(bi.net ~ sum) # probabilidad media de enlaces.

#b1cov -> efecto de una variable continua referente a un atributo de la planta (1 se refiere a filas o modo 1)
#b2factor -> efecto de una variable categorica (factor) referente a un atributo de los polinizadores (2 se refiere a columnas o modo 2)

# modelo completo con todas las variables (fenotipicas, microhabitat, vecindario)
...
# modelo con las variables fenotipicas
...
# modelo con las variables de microhabitat
...
# modelo con las variables de vecindario
...
ergm.fit1.init <- ergm(ergm1, response= "weight", reference= ~Poisson, control= control.ergm(init= ergm.fit1$coef))                 # Full model

ergm.fit1b <- ergm(ergm1b, response= "weight", reference= ~Poisson)    # Full model without func group
...
summary(ergm.fit1.init) 
mcmc.diagnostics(ergm.fit1.init) # For ergm() specifically, recent changes in the estimation algorithm mean that these 

summary(ergm.fit1b) 
mcmc.diagnostics(ergm.fit1b) # without functional groups
...
set.seed(42)
uni.ergm.fit0 <- ergm(uni.ergm0, response ="weight", reference= ~Poisson, control =control.ergm(MCMLE.maxit = 30)) 
uni.ergm.fit1 <- ergm(uni.ergm1, response ="weight", reference= ~Poisson, control =control.ergm(MCMLE.maxit = 30))   
uni.ergm.fit2 <- ergm(uni.ergm2, response ="weight", reference= ~Poisson, control =control.ergm(MCMLE.maxit = 30)) 
